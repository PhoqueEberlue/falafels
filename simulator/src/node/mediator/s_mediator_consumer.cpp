...
unique_ptr<operations::Operation> SMediatorConsumer::get_received_operation()
{
    return this->mq_received_operations->get_unique<operations::Operation>();
}

void SMediatorConsumer::put_to_be_sent_packet(filters::NodeFilter filter, const operations::Operation op)
{
    auto p = new Packet(filter, op);
    this->mq_to_be_sent_packets->put(p);
}

void SMediatorConsumer::put_async_to_be_sent_packet(filters::NodeFilter filter, const operations::Operation op)
{
    auto p = new Packet(filter, op);
...
unique_ptr<events::NetworkEvent> SMediatorConsumer::get_nm_event()
{
    return this->mq_nm_events->get_unique<events::NetworkEvent>();
}

void SMediatorConsumer::wait_all_async_comms()
{
