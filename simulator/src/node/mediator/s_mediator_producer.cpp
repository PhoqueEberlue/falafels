...
shared_ptr<Packet> SimulatedMediatorProducer::get_to_be_sent_packet()
{
...
AsyncWrapper SimulatedMediatorProducer::get_async_to_be_sent_packet()
{
    // return this->mq_to_be_sent_packets->get_async();
}


void SimulatedMediatorProducer::put_received_operation(const operations::Operation op)
{
...
void SimulatedMediatorProducer::put_nm_event(protocol::events::NetworkEvent *e)
{
    auto mess = this->mq_nm_events->put_async(e);
    this->async_messages->push(mess);
}

void SimulatedMediatorProducer::wait_all_async_comms()
{
