...
RingUniMOM::RingUniMOM(NodeInfo node_info) : MOM(node_info) {}

RingUniMOM::~RingUniMOM() {}
        ...
        case WAITING_REGISTRATION_CONFIRMATION:
            {
                ...
                if (auto *confirmation = get_if<operations::RegistrationConfirmation>(&packet->op))
                {
                    ...
                    // TODO: either the INetworkManager does it by itself, either we add a similar function
        ...
        case WAITING_REGISTRATION_REQUEST:
            {
                ...
                // }
                // TODO: no clue how to replace that
                            ...
                            if (p->original_src == this->get_my_node_name())
                            {
                                if (!this->cluster_connected_have_been_sent)
                                {
                                    ...
                                    this->mp->put_nm_event(
                                        new events::NetworkEvent {
                                            events::ClusterConnected { .number_client_connected=(uint16_t)p->nb_hops }
                                        }
                                    );
        ...
        case KILLING:
            {
                ...
                simgrid::s4u::this_actor::exit(); // TODO ???
            }
    }
}

void RingUniMOM::handle_registration_requests()
{
...
void RingUniMOM::send_registration_request()
{
    ...
    // Send the request
    this->nm->send_async(p);
}

void RingUniMOM::handle_registration_confirmation(const operations::RegistrationConfirmation &confirmation)
{
    ...
    this->mp->put_nm_event(
        new events::NetworkEvent { events::NodeConnected {} }
    );
}

void RingUniMOM::send_to_neighbour(const unique_ptr<Packet> &p, bool is_redirected) 
{
    p->dst = this->left_node.name;
    this->nm->send_async(p, is_redirected);
}


void RingUniMOM::handle_kill_phase()
{
