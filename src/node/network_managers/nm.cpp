...
void NetworkManager::send(Packet *packet, node_name dst)
{
    ...
    XBT_INFO("%s ---%s--> %s", packet->src.c_str(), packet->op_string.c_str(), packet->dst.c_str());
    auto receiver_mailbox = simgrid::s4u::Mailbox::by_name(packet->dst);
...
bool NetworkManager::send(Packet *packet, node_name dst, uint64_t timeout)
{
    ...
    XBT_INFO("%s ---%s--> %s (timeout: %lu)", packet->src.c_str(), packet->op_string.c_str(), packet->dst.c_str(), timeout);
...
simgrid::s4u::CommPtr NetworkManager::send_async(Packet *packet, node_name dst)
{
    ...
    XBT_INFO("%s ---%s--> %s (async)", packet->src.c_str(), packet->op_string.c_str(), packet->dst.c_str());
